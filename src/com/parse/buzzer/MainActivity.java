...
GooglePlayServicesClient.OnConnectionFailedListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (!provider.contains("gps")) { 
			...
			alertDialogBuilder.setMessage("Para funcionamento correto o Buzzer precisa que voc� ative o GPS. Deseja fazer isso agora?");
	...
	private void alertChecking(LatLng myLatLng) {
		...
		if (!mp.isPlaying()) { // caso j� esteja alertando, nao faz nada.
			while (it.hasNext()) {
				...
				if (marker.getSnippet() != null
					...
					if (!(marker.getSnippet().equals(ParseUser.getCurrentUser() //ToDo: corrigir isso, pois snippet n�o ser� usu�rio
						...
						// considerado
						// perigoso.
						//ou alertar caso exista alguma ocorr�ncia no raio (pensar em exibir apenas aquelas
						//que foram de ocorr�ncias no turno que o usu�rio est�
			...
			if (countOcc > numMaxOcc) {
				...
				alert.setMessage("�rea com "+ countOcc +" ocorr�ncia(s) registrada(s) no �ltimo m�s.");
